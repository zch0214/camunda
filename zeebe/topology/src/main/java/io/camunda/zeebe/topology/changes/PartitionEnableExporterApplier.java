...
public class PartitionEnableExporterApplier implements MemberOperationApplier {
  ...
  public ActorFuture<UnaryOperator<MemberState>> applyOperation() {
    ...
    partitionChangeExecutor
        .enableExporter(partitionId, exporterId)
        .onComplete(
            (ok, error) -> {
              if (error == null) {
                result.complete(
                    m ->
                        m.updatePartition(
                            partitionId, p -> p.updateConfig(c -> c.enableExporter(exporterId))));
              } else {
