...
public class ClusterEndpoint {
  ...
  public ResponseEntity<?> add(
    ...
    return switch (resource) {
      ...
      case forceRemove -> ResponseEntity.status(501).body("unsupported");
    };
  }
  ...
  public ResponseEntity<?> scale(
    ...
    return switch (resource) {
      ...
      case forceRemove -> {
        final var response =
            requestSender
                .forceOverwriteTopology(
                    new ForceOverwriteTopologyRequest(
                        ids.stream()
                            .map(String::valueOf)
                            .map(MemberId::from)
                            .collect(Collectors.toList())))
                .join();
        yield mapOperationResponse(response);
      }
    };
  }
    ...
    return switch (resource) {
      case brokers ->
          switch (subResource) {
            ...
            default -> throw new IllegalStateException("Unexpected value: " + subResource);
          };
      case partitions ->
          switch (subResource) {
