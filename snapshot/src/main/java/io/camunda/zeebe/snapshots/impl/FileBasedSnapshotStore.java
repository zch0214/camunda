...
public final class FileBasedSnapshotStore extends Actor
  ...
  private FileBasedSnapshot collectSnapshot(final Path path) throws IOException {
    ...
    //      LOGGER.debug(
    //          "Snapshot {} does not have a checksum file, which most likely indicates a partial
    // write"
    ...
    try {
      ...
      //        LOGGER.warn(
      //            "Expected snapshot {} to have checksum {}, but the actual checksum is {}; the
      // snapshot is most likely corrupted. The startup will fail if there is no other valid
      // snapshot and the log has been compacted.",
      //            path,
